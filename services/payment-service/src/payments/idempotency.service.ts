...
export class IdempotencyService {
  async find(manager: EntityManager, key: string): Promise<StoredResult | null> {
    const record = await manager.getRepository(IdempotencyKeyEntity).findOne({ where: { key } });
    if (!record) return null;
    return { replyTopic: record.replyTopic, replyPayload: record.replyPayload };
  }

  async save(manager: EntityManager, key: string, replyTopic: string, replyPayload: object): Promise<void> {
